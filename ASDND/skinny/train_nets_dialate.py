...
def train_distinguisher(
        cipher, diff, n_train_samples=10**7, n_val_samples=10**6, n_epochs=50, depth=5, n_neurons=64, kernel_size=3,
...
if __name__ == '__main__':
    # acc=np.load("./7round/train_acc.npy")
    # val_acc=np.load("./7round/val_acc.npy")
    # plt.rcParams['figure.figsize'] = (20,20)
    # plt.ylim(0.5,0.57)
    # plt.grid(True)
    # plt.plot(acc, '-', linewidth=3.0,label='train_acc')
    # plt.plot(val_acc, '-', linewidth=3.0,label='val_acc')
    #
    # # 设置坐标标签标注和字体大小
    ...
    #
    # plt.savefig('fig/Present_7r_acc.svg',
    ...
    # # plt.ylim(0.6,1)
    # plt.grid(True)
    # plt.plot(loss, '-', linewidth=3.0,label='train_loss')
    # plt.plot(val_loss, '-', linewidth=3.0,label='val_loss')
    # plt.xlabel('Epoch',fontsize=20)
    ...
    # # title = plot.title(filename + '\n_' + str(nb_traces) + 'trs_' + str(nb_attacks) + 'att', loc='center')
    #
    # plt.savefig('fig/Present_7r_loss.svg',
    #              format='svg', dpi=1200, bbox_extra_artists=(legend,), bbox_inches='tight')
    ...
    # plt.show()
    plt.savefig('fig/Present_7r.svg',
