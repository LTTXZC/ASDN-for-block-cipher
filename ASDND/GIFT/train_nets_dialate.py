...
def make_resnet(
    ...
    # conv0 = Conv(num_filters, kernel_size=3, padding='same', dilation_rate=1, kernel_regularizer=l2(reg_param))(conv0)
...
def train_distinguisher(
        ...
        n_filters=32, reg_param=10 ** -5, lr_high=0.002, lr_low=0.0001, cconv=False, calc_back=0,filename='fig'):
    ...
    np.save(wdir + 'h' + str(n_rounds) + 'r_depth' + str(depth) + '_val_acc_.npy', h.history['val_acc']);
    np.save(wdir + 'h' + str(n_rounds) + 'r_depth' + str(depth) + '_val_loss_.npy', h.history['val_loss']);
    ...
    # 训练集acc
    acc = h.history['acc']
    np.save("./dilated_models/5train_acc.npy", acc)
    # 测试集acc
    val_acc = h.history['val_acc']
    np.save("./dilated_models/5val_acc.npy", val_acc)
    # 训练集loss
    loss = h.history['loss']
    np.save("./dilated_models/5train_loss.npy", loss)
    # 测试集Loss
    val_loss = h.history['val_loss']
    np.save("./dilated_models/5val_loss.npy", val_loss)
    # plt.subplot(1, 2, 1)
    ...
    # plt.legend()
    # plt.show()
    # legend = plt.legend(loc='upper left', bbox_to_anchor=(1, 1), borderaxespad=1, ncol=1)
    #
    # plt.savefig('fig/' + filename + '_' + str(cipher_name) + 'trs_' + str(n_rounds) + 'loss.svg', format='svg',
    #              dpi=1200, bbox_extra_artists=(legend,), bbox_inches='tight')
    return net, h


if __name__ == '__main__':
    # acc=np.load("./dilated_models/5train_acc.npy")
    # val_acc=np.load("./dilated_models/5val_acc.npy")
    #
    # plt.rcParams['figure.figsize'] = (12,10)
    # plt.ylim(0.68,0.83)
    # plt.grid(False)
    # plt.plot(acc, '-', linewidth=3.0, label='训练集')
    # plt.plot(val_acc, '-', linewidth=3.0,label='验证集')
    #
    # # 设置坐标标签标注和字体大小
    ...
    #
    # plt.savefig('fig/Gift_5r_acc.svg',
    #              format='svg', dpi=1200, bbox_extra_artists=(legend,), bbox_inches='tight')
    loss=np.load("./dilated_models/5train_loss.npy")
    val_loss=np.load("./dilated_models/5val_loss.npy")
    plt.rcParams['figure.figsize'] = (12, 10)
    # plt.ylim(0.6,1)
    plt.grid(False)
    plt.plot(loss, '-', linewidth=3.0, label='训练集')
    plt.plot(val_loss, '-', linewidth=3.0,label='验证集')
    plt.xlabel('训练轮数（epoch)',fontsize=20)
    plt.ylabel('损失率', fontsize=20)
    ## 设置坐标刻度字体大小
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    ...
    plt.savefig('fig/Gift_5r_loss.svg',
                 format='svg', dpi=1200, bbox_extra_artists=(legend,), bbox_inches='tight')
    ...
    # plt.ylim(0.68,0.82,0.05)
    # # plt.grid(True)
    # plt.plot(acc, '-', linewidth=2.0,label='训练集')
    # plt.plot(val_acc, '-', linewidth=2.0,label='验证集')
    # # 设置坐标标签标注和字体大小
    # plt.xlabel('训练轮数（epoch)', fontsize=15)
    # plt.ylabel('准确率', fontsize=15)
    # plt.title('(a)精度曲线', y=-0.2,fontsize=15, loc='center')
    # # 设置坐标刻度字体大小
    ...
    #
    # plt.subplot(1,2,2)
    # plt.plot(loss, '-', linewidth=2.0,label='训练集')
    # plt.plot(val_loss, '-', linewidth=2.0,label='验证集')
    # # plt.grid(True)
    # plt.ylim(0.12, 0.20,0.003)
    # # 设置坐标标签标注和字体大小
    # plt.xlabel('训练轮数（epoch)',fontsize=15)
    # plt.ylabel('损失率', fontsize=15)
    # plt.title('(b)损失曲线', y=-0.2,fontsize=15, loc='center')
    # # 设置坐标刻度字体大小
    # plt.xticks(fontsize=15)
    # plt.yticks(fontsize=15)
    ...
    # fig.legend(lines, labels, loc='lower center',fontsize=15)
    # # plt.show()
    # plt.savefig('fig/Gift_5r.svg',
